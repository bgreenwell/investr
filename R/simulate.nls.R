#' @importFrom stats fitted rnorm runif
#' @keywords internal
simulate.nls <- function (object, nsim = 1, seed = NULL, ...) {
  ...
  vars <- stats::deviance(object) / stats::df.residual(object)  # residual variance
  if (!is.null(object$weights)) {
    vars <- vars/object$weights
  }
  val <- ftd + stats::rnorm(ntot, sd = sqrt(vars))  # simulated response values
