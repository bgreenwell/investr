# predFit matches results from PROC NLIN in SAS
...
# predFit works properly on 'special' nls fits --------------------------------
...
# predFit matches output from stats::predict -----------------------------------
...
# Working-Hotelling band: predFit reproduces example from Applied Linear
# Statistical Models (ALSM) -----------------------------------------------
...
expect_equivalent(wh_lm, wh_nls, tolerance = 1e-05)
expect_identical(unname(round(wh_lm$se.fit, digits = 2)), c(16.97, 9.92, 14.27))
expect_identical(round(wh_lm$fit, digits = 1), ans)

# se.fit = TRUE works for nls models with more than one predictor variable
# (regression test for issue #53: assign() only bound the first predictor
# name when there were several, breaking the symbolic gradient evaluation)

set.seed(101)
d53 <- transform(
  data.frame(pred1 = rep(1:10 / 10, 2), pred2 = rep(0:4 * 111, 4)),
  y = 56 * pred1 + pred2 * exp(-1.632 * pred1) + rnorm(20, 13, 24)
)
model53 <- nls(y ~ a * pred1 + pred2 * exp(-b * pred1), data = d53,
                start = list(a = 0, b = 0))

pred53 <- predFit(model53, se.fit = TRUE)
